...
function status($status) {
    return $status == 0 ? "<font color='red'>交易中</font>" : "<font color='blue'>交易成功</font>";
}
...
function pd_ywy1($id) {
    $fck_rs = M('fck')->field('ywy')->find($id);
    if ($fck_rs['ywy'] == 0) {
        return "<a href=\"javascript:void(0)\" onclick=\"if(confirm('确定升级吗?')) { window.url({$id});} return false;\">升级</a>";
    } else {
        return "";
    }
}

function getdengji($user_id) {
    $fee = M('fee');
    $fck = M('fck');
    ...
    if ($fck_rs['is_agent'] == 2)
        ...
        $fee_rs = $fee->field("s10")->find(1);
        $s11 = explode("|", $fee_rs['s10']);
        return $s11[$fck_rs['u_level'] - 1];
    }
}
